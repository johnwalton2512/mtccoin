...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MtcCoinTEST2\r");
            Send(hSocket, "WHO #MtcCoinTEST2\r");
        } else {
            // randomly join #MtcCoin00-#MtcCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #MtcCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MtcCoin%02d\r", channel_number).c_str());
        }
