MtcCoin-qt: Qt4 GUI for MtcCoin
===============================
...
Alternatively, install Qt Creator and open the `MtcCoin-qt.pro` file.

An executable named `MtcCoin-qt` will be built.
...
A warning for people using the *static binary* version of MtcCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of MtcCoin is linked against libdb5.3.
...
installed causes MtcCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
